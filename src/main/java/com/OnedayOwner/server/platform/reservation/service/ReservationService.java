...
public class ReservationService {
    ...
    public ReservationDto.ReservationDetailForCustomer registerReservation(
        ...
        //예약 시간 검증
        if(!checkValidationReservationTime(reservationForm.getReservationTime(), reservationForm.getNumberOfPeople(), reservationForm.getPopupId())) {
    ...
    /*
    예약 시간 검증 로직
    예약 시간이 등록된 시간과 맞는지, 현재시간 이후인지, 예약인원 + 현재인원이 Max인원보다 작거나 같은지 check
     */
    private boolean checkValidationReservationTime(LocalDateTime reservationTime, int numberOfPeople, Long popupId) {
